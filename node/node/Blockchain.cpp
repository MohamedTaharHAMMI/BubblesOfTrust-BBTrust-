...
Blockchain::Blockchain(const QString& ipAddr, const QString& p) : ipAddress(ipAddr), port (p), coinbase(""), myExternalAddress ("")
{
...
void Blockchain::setContractAddress (const QString& _contractAddress)
{
...
// This is just the right representation (we does not require an encode operation)
    ...
    if (encodedValue.length() != _AddrTypeLen) {
        qDebug() << "Error:Blockchain::EncodeAddress, the address is not correct (length should be equal to 20 bytes" << encodedValue.length()  << " != " << _AddrTypeLen ;
        encodedValue = "" ;
        goto error ;
    }
...
QString Blockchain::EncodeUint256 (const QString & value) {
    int32_t rc = 0 ;
    if (value.length() != 64) { // (256x2)/8
        qDebug() << "Blockchain::EncodeUint256, value.length != 64" ;
        return NULL ;
    }
